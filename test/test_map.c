...
static void set_config(FILE **log_stream) { *log_stream = fopen("logs/test_map.log", "w"); }
...
static void test_map_iterate_keys_values(void) {
    map m = Map.new(8);

    // Set up test data with heap-allocated keys
    char *keys[5];
    keys[0] = strdup("apple");
    keys[1] = strdup("banana");
    keys[2] = strdup("cherry");
    keys[3] = strdup("date");
    keys[4] = strdup("elderberry");

    // Insert with known values
    Map.set(m, keys[0], strlen(keys[0]), 100);
    Map.set(m, keys[1], strlen(keys[1]), 200);
    Map.set(m, keys[2], strlen(keys[2]), 300);
    Map.set(m, keys[3], strlen(keys[3]), 400);
    Map.set(m, keys[4], strlen(keys[4]), 500);

    // Create iterator
    sparse_iterator it = Map.create_iterator(m);
    Assert.isNotNull(it, "Iterator should be created");
    ...
    while (SparseIterator.next(it)) {
        ...
        Assert.isTrue(SparseIterator.current_value(it, (object *)&entry) == OK,
                      "Should get current entry");
        Assert.isNotNull(entry, "Entry should not be NULL");

        // Verify entry has valid key and value
        Assert.isNotNull((object)entry->key, "Entry key should not be NULL");
        Assert.isTrue(entry->key_len > 0, "Entry key length should be positive");

        // Match against known keys
        bool found = false;
        for (usize i = 0; i < 5; i++) {
            if (entry->key_len == strlen(keys[i]) &&
                memcmp(entry->key, keys[i], entry->key_len) == 0) {
                found = true;
                visited[i] = true;

                // Verify value matches
                usize expected_value = (i + 1) * 100;
                Assert.isTrue(entry->value == expected_value, "Entry value should match expected");
                break;
            }
        }
        Assert.isTrue(found, "Entry key should match one of the inserted keys");
        entry_count++;
    }

    // Verify all entries were visited
    Assert.isTrue(entry_count == 5, "Should iterate over all 5 entries");
    for (usize i = 0; i < 5; i++) {
        Assert.isTrue(visited[i], "All entries should be visited");
    }

    // Test iterator reset
    SparseIterator.reset(it);
    Assert.isTrue(SparseIterator.next(it), "Iterator should work after reset");
