...
static void test_farray_clear(void) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        Assert.areEqual(&test_values[i], &stored_value, INT, "Pre-clear value mismatch at index %d",
                        i);
    }
...
static void test_farray_set_value(void) {
    ...
    for (int i = 0; i < 5; i++) {
        Assert.areEqual(&(int){0}, &(int){FArray.set(arr, i, element_size, &values[i])}, INT,
                        "FArray set ERRed at index %d", i);
    }
...
static void test_farray_get_value(void) {
    ...
    for (int i = 0; i < 5; i++) {
        int element = 0;
        Assert.isTrue(FArray.get(arr, i, element_size, &element) == 0,
                      "FArray get ERRed at index %d", i);
...
static void test_farray_remove_at(void) {
    ...
    Assert.areEqual(&(int){0}, &(int){FArray.remove(arr, 2, element_size)}, INT,
                    "FArray remove ERRed at index 2");
    ...
    for (int i = 0; i < initial_capacity; i++) {
        ...
        Assert.areEqual(&expected_values[i], &actual_value, INT,
                        "FArray remove_at mismatch at index %d", i);
    }
