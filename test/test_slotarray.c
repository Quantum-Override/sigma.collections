...
static void test_slotarray_get_value(void) {
    slotarray sa = SlotArray.new(5);
    int *expValue = malloc(sizeof(int));
    *expValue = 99;
...
static void test_slotarray_remove_at(void) {
    ...
    *expValue = 123;
    ...
    Assert.areEqual(&(int){-1}, &result, INT,
                    "SlotArray get_at should ERR after remove at handle %d", handle);
...
static void test_slotarray_capacity(void) {
    ...
    *p2 = 2;
    ...
    Assert.areEqual(&(int){10}, &(int){capacity}, INT,
                    "Capacity should remain 10 after adding 2 items");
...
static void test_slotarray_clear(void) {
    ...
    *p2 = 2;
    ...
    object retrieved;
    Assert.areEqual(&(int){0}, &(int){SlotArray.get_at(sa, h1, &retrieved)}, INT,
                    "Get h1 ERRed before clear");
    Assert.areEqual(&(int){0}, &(int){SlotArray.get_at(sa, h2, &retrieved)}, INT,
                    "Get h2 ERRed before clear");

    // Clear
    SlotArray.clear(sa);

    // Now they should be gone
    Assert.areEqual(&(int){-1}, &(int){SlotArray.get_at(sa, h1, &retrieved)}, INT,
                    "Get h1 should ERR after clear");
    Assert.areEqual(&(int){-1}, &(int){SlotArray.get_at(sa, h2, &retrieved)}, INT,
                    "Get h2 should ERR after clear");
...
static void test_slotarray_stress(void) {
    ...
    for (usize i = 0; i < 10; i += 2) {
        ...
        free(values[i]);   // Free the actual memory
    ...
    for (usize i = 1; i < 15; i += 2) {  // Check odd indices (not removed)
        if (valid[i]) {
            ...
            Assert.areEqual(&(int){i}, (int *)retrieved, INT,
                            "Retrieved value content %zu mismatch", i);
        }
    }
    ...
    for (usize i = 15; i < 20; i++) {
        ...
        *values[i] = (int)(i + 100);
        ...
        object retrieved;
        Assert.areEqual(&(int){0}, &(int){SlotArray.get_at(sa, (usize)handles[i], &retrieved)}, INT,
                        "Final get %zu ERRed", i);
...
static void test_slotarray_from_pointer_array(void) {
    ...
    object retrieved;
    for (int i = 0; i < 5; i++) {
        Assert.areEqual(&(int){OK}, &(int){SlotArray.get_at(sa, i, &retrieved)}, INT,
                        "SlotArray get_at ERRed");
...
// test attaching a parray as slotarray
static void test_parray_as_slotarray(void) {
    parray arr = PArray.new(5);
    int values[] = {100, 200, 300, 400, 500};
    for (int i = 0; i < 5; i++) {
        PArray.set(arr, i, (addr)values[i]);
    }
    // create slotarray view
    slotarray sa = PArray.as_slotarray(arr);
    Assert.isNotNull(sa, "PArray as_slotarray ERRed");
